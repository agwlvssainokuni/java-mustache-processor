...
- **依存関係**: すべてMaven Centralから解決（`build.gradle.kts`/`cherry-mustache-core/build.gradle.kts`/`cherry-mustache-cli/build.gradle.kts`に記載。個別インストール作業は不要）
...
- **生成される成果物**:
  - `cherry-mustache-core/build/libs/cherry-mustache-core-<version>.jar`（ライブラリ本体）
  - `cherry-mustache-cli/build/libs/cherry-mustache-cli-<version>.jar`（`core`への依存を含まない通常jar）
  - `cherry-mustache-cli/build/libs/cherry-mustache-cli-<version>-all.jar`（Gradle Shadow pluginによる実行可能fat jar。依存関係一式を同梱）
- **確認済みの警告**: `Consider enabling configuration cache...`というGradleの一般的な推奨メッセージのみ（ビルドの成否には影響しない）

### 4. fat jarの動作確認（任意）
```bash
java -jar cherry-mustache-cli/build/libs/cherry-mustache-cli-*-all.jar --help
```
