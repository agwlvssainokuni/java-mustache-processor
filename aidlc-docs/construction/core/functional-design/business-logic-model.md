...
## 4. Testable Properties（Property-Based Testing拡張: PBT-01）

| コンポーネント | 性質カテゴリ | 内容 |
|---|---|---|
| Parser + Renderer全体 | Oracle | 公式Mustache specテストスイート（BR-2のスコープ内のYAMLケース）を基準実装（oracle）とし、任意の入力テンプレート・データについてパース→レンダリング結果が期待値と一致することを検証する |
| HTMLエスケープ処理（BR-1） | Invariant | エスケープ後の文字列に生の`<`, `>`, `"`が出現しないこと。対象4文字（`& < > "`）を含まない任意の入力文字列は、エスケープ処理の前後で不変であること |
| SectionNode（真かつリストの場合） | Invariant | 出力はリストの各要素についてセクション内容を1回ずつレンダリングした結果の連結と一致する（要素数に応じた繰り返し構造を保つ） |
| Context.resolve（ドット表記、BR-6） | Invariant | 深さNのドット表記`a.b.c...`の解決結果は、各セグメントを1つずつ手動でMap.get/POJOプロパティアクセスして辿った結果と常に一致する |
| デリミタ変更（Set Delimiter、BR-11） | Invariant | 同じ論理的なタグ列を、デフォルトデリミタで書いたテンプレートと、任意のカスタムデリミタに変更して書いたテンプレートとで、レンダリング結果が一致する |
| パーシャル解決（循環なし、BR-9） | Idempotence相当 | 循環していない同一パーシャルを複数箇所から参照しても、各参照箇所は互いに独立して同じ入力に対し常に同じレンダリング結果を生成する |
| Lambda（変数/セクション、BR-2.4） | Oracle | `~lambdas.yml`の公式specケースを基準（oracle）として、Lambda実行結果の再パース・再レンダリング・エスケープ適用有無が仕様と一致することを検証する |

**Round-tripについて**: coreは「テンプレート＋データ→出力文字列」の一方向変換のみを提供し、出力文字列からテンプレートやデータを復元する逆変換APIを持たない（要件上も不要）。そのため、coreの範囲では典型的なRound-trip性質（`f_inv(f(x)) == x`）に該当する操作は無いと判断する（PBT-01該当なしの明記）。CLI（Unit 2）のJSON/YAMLデータ読み込みについては、Unit 2のFunctional Designで別途評価する。
