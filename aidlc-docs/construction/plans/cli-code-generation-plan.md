...
- [ ] `cli/build.gradle.kts`に以下を追加:
  - Jackson依存（`com.fasterxml.jackson.core:jackson-databind`, `com.fasterxml.jackson.dataformat:jackson-dataformat-yaml`、`implementation`スコープ、tech-stack-decisions.md）
  - Gradle Shadow plugin（`com.gradleup.shadow`）の適用、Manifestの`Main-Class`設定
  - OWASP Dependency-Checkプラグイン適用（`core/build.gradle.kts`と同じバージョン・設定方針）
  - SLF4J（`org.slf4j:slf4j-api`、`implementation`）、`slf4j-simple`（`testRuntimeOnly`）
  - `testRuntimeOnly("org.junit.platform:junit-platform-launcher")`
- [ ] `cli/dependency-check-suppressions.xml`（`core`と同様の空の抑制ファイル）
- [ ] `cli/src/main/java/cherry/mustache/cli/`, `cli/src/test/java/cherry/mustache/cli/` ディレクトリ作成
- [ ] `./gradlew :cli:build`でビルド疎通確認（Main未実装のためこの時点ではコンパイルのみ通ることを確認）

### Step 2: 例外・ドメインモデルの生成（domain-entities.md）
- [ ] `cherry.mustache.cli.ArgumentException`（`RuntimeException`継承、`message`コンストラクタ）
- [ ] `cherry.mustache.cli.ExitCode`（enum: `SUCCESS(0)`, `ARGUMENT_ERROR(1)`, `PARSE_ERROR(2)`, `RENDER_ERROR(3)`, `IO_ERROR(4)`、`int code()`）
- [ ] `cherry.mustache.cli.CliArguments`（イミュータブル値オブジェクト。`templateArgs`, `dataArg`, `format`, `outputPath`, `partialDir`, `helpRequested`）
- 対応BR: BR-2, BR-8

### Step 3: ArgumentParserの生成（business-rules.md BR-2）
- [ ] `cherry.mustache.cli.ArgumentParser`（`CliArguments parse(String[] args)`）
...
- [ ] `cherry.mustache.cli.DataLoader`（`Map<String, Object> load(CliArguments args, InputStream stdin)`）
  - Jacksonの`ObjectMapper`（JSON用）と`YAMLMapper`/`ObjectMapper`+`YAMLFactory`（YAML用）を用いて`Map<String, Object>`に変換
  - データ形式判定順序: `--format`優先 → ファイル拡張子（`.json`/`.yaml`/`.yml`） → 判定不能なら`ArgumentException`
...
- [ ] `cherry.mustache.cli.OutputWriter`（`void write(String content, CliArguments args, PrintStream stdout)`）
  - `outputPath`指定時は常に上書きでファイル書き込み（UTF-8固定）、未指定時は`stdout`へ書き込み
...
- [ ] `cherry.mustache.cli.CliRunner`（`ExitCode run(String[] args, InputStream in, PrintStream out, PrintStream err)`）
  - オーケストレーションフロー（引数解析→`--help`分岐→データ読込→パーシャルディレクトリ決定→テンプレートごとのコンパイル/レンダリング→連結→出力）を実装
  ...
  - `--help`時のUsage表示
  - パーシャル解決ディレクトリ決定ロジック（`--partial-dir`優先、未指定時はテンプレートごとの親ディレクトリ、標準入力由来テンプレートはパーシャル未解決固定、business-logic-model.md 5節）
  - エラーメッセージは内部詳細を含まない簡潔な文言を`err`へ、詳細はSLF4Jログへ（BR-9）
- 対応BR: BR-2〜BR-9 / 対応NFR: 全Resilience/Security Patterns

### Step 7: Mainの生成
- [ ] `cherry.mustache.cli.Main`（`public static void main(String[] args)`。`CliRunner.run(args, System.in, System.out, System.err)`を呼び出し`System.exit(exitCode.code())`）
- [ ] `cli/build.gradle.kts`の`application.mainClass`が`cherry.mustache.cli.Main`を指していることを確認（既存プレースホルダーの通り）
