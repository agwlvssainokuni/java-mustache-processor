...
- テンプレート・パーシャルファイルの標準拡張子は `.mustache` とする
...
- **パーシャル解決**: テンプレートファイルと同一ディレクトリ内からパーシャル名＋`.mustache`拡張子でファイルを探すことを既定とし、コマンドライン引数で明示的にパーシャル用ディレクトリを指定して上書きすることも可能とする
...
- エラーメッセージ・ヘルプテキストの言語: 英語を既定とする。国際化（i18n）対応は将来の拡張とし、今回のスコープには含めない
...
- SECURITY-03（アプリケーションレベルログ）: CLIは単発実行のプロセスであり集中ログサービスへの出力対象を持たないため、基本的に **N/A** と判定する見込み（最終判定は構築ステージで実施）
- 特に該当する可能性が高いルール:
